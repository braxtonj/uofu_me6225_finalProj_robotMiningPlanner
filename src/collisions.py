#!/usr/bin/env python
'''
collisions.py - Helper classes and functions 
for performing graph search operations for planning.
Adapted from Project 1: Graph Search
...
class RectRobot:
    ...
    def draw(self, q, color='b', show=False, base_color='b', theTitle=''):
        '''
...
class RevoluteRobotChain:
    ...
    def draw(self, q, color='b', show=False, base_color='g',theTitle=''):
        '''
        Draw the robot with the provided configuration
        '''
        plotter.hold(True)
...
class PolygonEnvironment:
    ...
    def read_env(self, env_file_path, start_pos='()', goal_pos='()'):
        '''
        ...
        for l in file_infos:
            ...
            if line_info[0].startswith('#'):
                continue
            if line_info[0] == _START and start_pos != '()' and '.csv' not in start_pos:
                self.start = np.array(start_pos)
            elif line_info[0] == _GOAL and goal_pos != '()' and '.csv' not in goal_pos:
                self.goal = np.array(goal_pos)
            else:
                self.line_parser[line_info[0]](line_info[1:])
        print self.line_parser
    ...
    def line_line_collision(self, l1, l2, eps=0.00001, dbg=False):
        '''
    ...
    def draw_env(self, q=None, show=True, theTitle=''):
        '''
        ...
        # Draw robot
        if q is not None:
            self.robot.draw(q, theTitle=theTitle)
    ...
    def saveCanvasFromFig(self,fig,ax):
        self.figToSave = fig.canvas.copy_from_bbox(ax.bbox)

    def draw_plan(self, plan, planner, start_pos=None, goal_pos=None, dynamic_tree=False, dynamic_plan=False, show=True, save=True, save_fldr='out', i_fig = 0, theTitle=''):
        '''
        ...
        self.draw_env(show=False, theTitle=theTitle)
        ...
        if save:
            fig = plotter.gcf()
            ft_fig = 'jpg'
            fn_fig = os.path.join(save_fldr,'{}.{}'.format(i_fig,ft_fig))
            fig.savefig(fn_fig)
        if show:
            plotter.title(theTitle)
            plotter.show()
        ...
        if planner.T[0].compareNodes(planner.T[0].root.state,self.goal,planner.epsilon):
            (planner.T[0],planner.T[1]) = planner.swapTrees(planner.T[0],planner.T[1])

        if planner is not None:
            Qs1, edges1 = planner.T[0].get_states_and_edges()
            try:
                Qs2, edges2 = planner.T[1].get_states_and_edges()
                edges = edges1+edges2
            except:
                edges = edges1
            e1Len = len(edges1)
            ...
            for i, e in enumerate(edges):
                ...
                if i>e1Len-1:
                    ...
                    clr = 'c'
                plotter.plot([e0[0], e1[0]], [e0[1], e1[1]], '{}'.format(clr))
                plotter.plot([e0[0], e1[0]], [e0[1], e1[1]], '{}.'.format(clr))
                if dynamic_tree:
                    if save:
                        fig = plotter.gcf()
        ...
        self.saveCanvasFromFig(plotter.gcf(), plotter.gca())

        if plan is not None:
            self.robot.draw(plan[0], color='g', theTitle='')
            for i in xrange(len(plan)):
                ...
                if dynamic_plan:
                    if save:
                        ...
                        fn_fig = os.path.join(save_fldr, '{}.{}'.format(i_fig, ft_fig))
                        fig.savefig(fn_fig)
                    plotter.gcf().canvas.restore_region(self.figToSave)
                    plotter.pause(0.01)
            self.robot.draw(plan[-1], color='r', theTitle=theTitle)
    ...
    def draw_conGraph(self, graph, show=True, save=True, save_fldr='out', i_fig = 0, theTitle=''):
        '''
        Draw the environment with an overlaid plan.
        plan - sequence of configurations to be drawn as plan (not drawn if pass in None)
        planner - a planner which has a function of the form
        ...
        Qs1, edges1 = graph.T[0].get_states_and_edges()
        try:
            Qs2, edges2 = graph.T[1].get_states_and_edges()
            edges = edges1+edges2
        except:
            edges = edges1
        e1Len = len(edges1)
